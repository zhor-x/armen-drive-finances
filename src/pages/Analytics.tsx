              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    labelLine={false}
                    label={(entry) => `${entry.name}: ${((entry.amount / summary.totalIncome) * 100).toFixed(0)}%`}
                    outerRadius={80}
                    fill="#8884d8"
                    dataKey="amount"
                  >
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    labelLine={false}
                    label={(entry) => `${entry.name}: ${((entry.amount / summary.totalExpense) * 100).toFixed(0)}%`}
                    outerRadius={80}
                    fill="#8884d8"
                    dataKey="amount"
                  >
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="p-4 rounded-lg bg-success/10 border border-success/20">
              ...
              <p className="text-2xl font-bold text-success">
                {formatAmount(incomeData.length > 0 ? summary.totalIncome / incomeData.reduce((sum, item) => sum + 1, 0) : 0)}
              </p>
            </div>
            <div className="p-4 rounded-lg bg-destructive/10 border border-destructive/20">
              <p className="text-sm text-muted-foreground mb-1">Միջին ծախս մեկ գործարքով</p>
              <p className="text-2xl font-bold text-destructive">
                {formatAmount(expenseData.length > 0 ? summary.totalExpense / expenseData.reduce((sum, item) => sum + 1, 0) : 0)}
              </p>
            </div>
